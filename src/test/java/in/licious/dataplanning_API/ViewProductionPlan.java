...
public class ViewProductionPlan {
	@Test
	public void viewProductionPlanTest() throws IOException, ParseException
	{
		...
		requestParams1.put("week","46" ); 
		requestParams1.put("year","2018" );
		requestParams1.put("ck", "CK_001");
		requestParams1.put("date","2018-11-16" );
		...
		String prd="pr_5785b9065d7e1";
		System.out.println(dateString);
		
		if(response1.getBody().asString().contains(prd))
		{
			if(response1.getBody().asString().contains(result1))
			{
			if(response1.getBody().asString().contains(numberAsString))
			{
			System.out.println("Production Plan pass for "+prd+" "+result1+" "+numberAsString);
			}
			}
		}else
		{
